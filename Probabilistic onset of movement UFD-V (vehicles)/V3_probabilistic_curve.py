...
Lz_list[Lz_list<1.45] = 1.45 # Never shorter.

# 3. Obtain rhob:
A, B, RMSE = 16.686, 38.175, 10.088
dev = RMSE*np.sqrt(3)
...
M_list[M_list<1000] = 1000

# 4. Obtain clearance zc:
zc_list =  np.random.uniform(0.118,0.42,Nr) # fa.Linear(Lz_list, A, B) + np.random.normal(0,RMSE,Nr)
...
for k in range(0, Nr):
    [hcrit_flot[k], vcrit_sliding[k,:], hcrit_sliding[k,:], vcrit_toppling[k,:], hcrit_toppling[k,:]] = \
        fa.get_stability_curves_V3(M_list[k],Lx_list[k],Ly_list[k],Lz_list[k],
...
folder_s = "V3-results"
np.save(folder_s +"\\"+"V3_vcrit.npy",vcrit_sliding)
np.save(folder_s +"\\"+"V3_hcrit.npy",hcrit)
np.save(folder_s +"\\"+"V3_vcrit_sliding.npy",vcrit_sliding)
np.save(folder_s +"\\"+"V3_hcrit_sliding.npy",hcrit_sliding)
np.save(folder_s +"\\"+"V3_vcrit_toppling.npy",vcrit_toppling)
np.save(folder_s +"\\"+"V3_hcrit_toppling.npy",hcrit_toppling)
plt.savefig(folder_s +"\\"+'V3_curve.pdf', dpi=600, format='pdf',  bbox_inches='tight')
plt.savefig(folder_s +"\\"+'V3_curve.png', dpi=600, format='png',  bbox_inches='tight')
plt.savefig(folder_s +"\\"+'V3_curve.svg', dpi=600, format='svg',  bbox_inches='tight')
